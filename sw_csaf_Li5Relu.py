...
class ResNNRTMC: 
    ...
    def loss_energy(self,  input_data, output_pred ): 
        F_net, sum_Cphr_gdp = self.energy_flux_HR_sw(input_data , output_pred) 
        return self.loss_fn(F_net,sum_Cphr_gdp) 

    def energy_flux_HR_sw(self, Input, Output): 
        # ! rsdt is the 103th input var
        F_sw_toa_do = Input[:,-1]/self.nor_para['input_scale'][-1] + self.nor_para['input_offset'][-1]
        F_sw_toa_up = Output[:,0]/self.nor_para['output_scale'][0] + self.nor_para['output_offset'][0]
        F_sw_sfc_do = Output[:,1]/self.nor_para['output_scale'][1] + self.nor_para['output_offset'][1]
        F_sw_sfc_up = Output[:,2]/self.nor_para['output_scale'][2] + self.nor_para['output_offset'][2] 
        F_net = F_sw_toa_do + (- F_sw_toa_up + F_sw_sfc_up - F_sw_sfc_do) 
...
if __name__ == '__main__': 
    ...
    Exp_name = 'sw_csaf_Li5Relu_cluster' 
    ...
    get_AM4_data_sw(out_filelist, inp_filelist, condition='csaf', month_sel = None, day_sel = [1,7]) 
    ...
    nor_para, input_array, output_array   = data_std_normalization_sw(input_array_ori, output_array_ori, nor_para) 
    ...
    ind_train, ind_test = split_train_test_sample(input_array.shape[0], test_ratio=0.3, rng=rng) 
    ...
    for i in range(run_num, total_run_num): 
        loss = []
        batch_size = max(8000, 8000*(i+1)**2)
        print(f'Train info >> run: {i+1} lr_sta: {lr_sta:7.1e}, batch size: {batch_size}')
        ...
        PATH =  exp_dir+f'/restart.{i+1:02d}.pth'
