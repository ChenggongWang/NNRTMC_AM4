...
class NNRTMC_NN: 
    ...
    def train(self, train_batch_indice, input_torch, output_torch, eng_loss_frac=None, optimizer=None):
        ...
        for i, batch_ids in enumerate(train_batch_indice):
            ...
            loss = loss_data
            if eng_loss_frac != None: loss += loss_ener*eng_loss_frac
            # Backpropagation
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
if __name__ == '__main__': 
    ...
    Exp_name = f'sw_{sky_cond}_Li5Relu_E{eng_loss}' 
    ...
    if eng_loss != 'Y':
        ...
        eng_loss_frac = 1e-4 # lower loss weight for csaf
    else:
        eng_loss_frac = 1e-3
    ...
    get_AM4_data_sw(out_filelist, inp_filelist, condition=sky_cond, month_sel = None, day_sel = [1, 7]) 
